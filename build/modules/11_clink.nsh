...
Section "Clink" section_clink
  ...
  ${If} ${RunningX64}
    ...
    ExecShell "open" "$DIR_modules\$GitHub_Repository\clink_x64.exe" 'autorun install --nolog --profile \"$DIR_config\clink\" --quiet' SW_HIDE
  ${Else}
    ExecShell "open" "$DIR_modules\$GitHub_Repository\clink_x86.exe" 'autorun --allusers uninstall' SW_HIDE
    ExecShell "open" "$DIR_modules\$GitHub_Repository\clink_x86.exe" 'autorun install --nolog --profile \"$DIR_config\clink\" --quiet' SW_HIDE
  ${EndIf}
